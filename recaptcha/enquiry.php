...
if($_POST){
    ...
    $used_email = filter_var($_POST["email"], FILTER_SANITIZE_EMAIL);
    $website = filter_var($_POST["website"], FILTER_SANITIZE_STRING);
    $yourrole = filter_var($_POST["yourole"], FILTER_SANITIZE_STRING);
    $content   = filter_var($_POST["content"], FILTER_SANITIZE_STRING);
    $trackedcountry = filter_var($_POST["trackedcon"], FILTER_SANITIZE_STRING);
    $trackedstate = filter_var($_POST["trackedstate"], FILTER_SANITIZE_STRING);
    $trackedcity = filter_var($_POST["trackedcity"], FILTER_SANITIZE_STRING);
    $trackedaddress = filter_var($_POST["trackedaddress"], FILTER_SANITIZE_STRING);
    $trackedip = filter_var($_POST["trackedIpaddress"], FILTER_SANITIZE_STRING);
	...
	if(!empty($empty)) {
		$output = json_encode(array('type'=>'error', 'text' => implode(", ",$empty) . ' Required!', 'values' => $_POST['g-recaptcha-response']));
        die($output);
	}
	
	if(!filter_var($used_email, FILTER_VALIDATE_EMAIL)){ //email validation
	    $output = json_encode(array('type'=>'error', 'text' => '<b>'.$used_email.'</b> is an invalid Email, please correct it.'));
		die($output);
	}
	...
	$mailBody='<table width="100%" border="0" align="left" cellpadding="0" cellspacing="0">
			...
			<tr><td>&nbsp;</td></tr>
              ...
              <tr>';
                    ...
                    <td width="76%" align="left">'.$used_email.'</td>
                  </tr>
									<tr>
                  ...
                  <tr>
                    <td width="24%"><strong>Tracked IP :</strong></td>
                    <td width="76%" align="left">'.$trackedip.'</td>
                  </tr>
                  <tr>
                    <td width="24%"><strong>Visited Page :</strong></td>
                    <td width="76%" align="left">'.$currenturl.'</td>
                  </tr>';
								$mailBody.='</table>
						</td>
          </tr>
	...
	if (mail($toEmail, "Enquiry Mail", $mailBody, $mailHeaders)) {
	    ...
	    global $wpdb;
	    $table_name = $wpdb->prefix . "location_track";
        $wpdb->insert( $table_name, array(
            ...
            'message' => $content,
            'tracked_country' => $trackedcountry,
            'tracked_city' => $trackedcity,
            'tracked_ipaddress' => $trackedip,
            'url' => $currenturl,
        ) );
	    $output = json_encode(array('type'=>'message', 'text' => 'Hi '.$user_name.$used_email.', thank you for the comments. We will get back to you shortly.'));
	    die($output);
